<UiSection asideRight border="all" intent="white">
  ...
  <svelte:fragment slot="aside">
    <UiImg {image} {widths} {sizes} class={cImg} />
    <UiFeatures {features} />
  </svelte:fragment>
...
<script lang="ts">
  import UiFeatures from '~/components/ui-features.svelte';
  import UiImg from '~/components/ui-img.svelte';
  import UiSection from '~/components/ui-section.svelte';
  import UiText from '~/components/ui-text.svelte';
  ...
  $: ({entry} = data);
  $: ({description, features, image, title} = entry);
</script>
